...
(function(){
  ...
  window.aiAnalyze=function(){
    ...
    q('aiSummary').innerHTML='<b>🤖 Rekomendasi AI:</b> '+high.length+' customer prioritas A belum dihubungi. Mulai dari mereka terlebih dahulu.';
    q('aiResult').innerHTML='<b>Ringkasan:</b><br>Total '+total+' • Belum dihubungi '+fresh.length+' • Follow Up '+fu.length+' • Respon '+resp.length+' • Deal '+deals.length+'<br><br><b>🎯 Customer yang disarankan:</b><br>'+high.slice(0,8).map((x,i)=>(i+1)+'. '+esc(x.name)+' — '+esc(x.city)+' — '+esc(x.phone)).join('<br>')+(high.length>8?'<br>...':'')+'<br><br><span class="muted">Langkah: pilih customer → cek pesan → buka WA Business → sob tekan KIRIM.</span>';
  };
  window.aiFollowups=function(){
    ...
    q('aiResult').innerHTML=list.length?list.slice(0,12).map((x,i)=>(i+1)+'. <b>'+esc(x.name)+'</b> — '+esc(x.city)+' — jadwal: '+esc(x.nextFollow||'belum diatur')).join('<br>')+'<br><br><span class="muted">AI hanya memberi rekomendasi. Pengiriman WhatsApp tetap manual.</span>':'Belum ada customer yang dijadwalkan Follow Up.';
  };
