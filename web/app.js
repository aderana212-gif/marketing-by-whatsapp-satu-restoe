...
function backupJSON(){const payload={format:'SatuRestoe Marketing Backup',version:2,exported_at:new Date().toISOString(),contacts:contacts.map(c=>({contact:c,status:statuses.get(c.id)||null}))};downloadText('SatuRestoe_Marketing_Backup.json',JSON.stringify(payload,null,2),'application/json;charset=utf-8');$('appMsg').textContent='Backup JSON seluruh customer dan history sudah dibuat.'}
...
$('todayBtn').onclick=()=>filterFollow('today');$('overdueBtn').onclick=()=>filterFollow('overdue');$('resetViewBtn').onclick=resetView;
